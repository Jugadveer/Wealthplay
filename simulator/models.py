...
class VirtualPortfolio(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='virtual_portfolio')
    cash_balance = models.DecimalField(max_digits=15, decimal_places=2, default=50000.00)  # Start with ₹50,000
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def total_value(self, current_prices):
        """Calculate total value including cash and assets based on live prices passed in dict"""
        asset_value = sum(
            asset.quantity * current_prices.get(asset.symbol, asset.average_buy_price)
            for asset in self.assets.all()
        )
        return float(self.cash_balance) + float(asset_value)

class PortfolioAsset(models.Model):
    portfolio = models.ForeignKey(VirtualPortfolio, on_delete=models.CASCADE, related_name='assets')
